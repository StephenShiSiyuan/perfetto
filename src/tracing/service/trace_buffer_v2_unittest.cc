...
class TraceBufferV2Test : public testing::Test {
 public:
  ...
  void ResetBuffer(
      size_t size_,
      TraceBufferV2::OverwritePolicy policy = TraceBufferV2::kOverwrite) {
  ...
  bool TryPatchChunkContents(ProducerID p,
                             WriterID w,
                             ChunkID c,
                             std::vector<TraceBufferV2::Patch> patches,
  ...
  static std::vector<FakePacketFragment> ReadPacket(
      const std::unique_ptr<TraceBufferV2>& buf,
      TraceBufferV2::PacketSequenceProperties* sequence_properties = nullptr,
      bool* previous_packet_dropped = nullptr) {
    std::vector<FakePacketFragment> fragments;
    TracePacket packet;
    TraceBufferV2::PacketSequenceProperties ignored_sequence_properties{};
...
TEST_F(TraceBufferV2Test, Fragments_PreserveUID) {
  ...
  CreateChunk(ProducerID(1), WriterID(1), ChunkID(1))
      ...
      .CopyIntoTraceBuffer();
  trace_buffer()->BeginRead();
  TraceBufferV2::PacketSequenceProperties sequence_properties;
...
TEST_F(TraceBufferV2Test, Clone_CommitOnlyUsedSize) {
  ...
  auto is_only_first_page_mapped = [&](const TraceBufferV2& buf) {
