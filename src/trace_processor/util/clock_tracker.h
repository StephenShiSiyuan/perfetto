...
#ifndef SRC_TRACE_PROCESSOR_UTIL_CLOCK_TRACKER_H_
#define SRC_TRACE_PROCESSOR_UTIL_CLOCK_TRACKER_H_
...
class ClockSynchronizer {
 public:
  ...
  explicit ClockSynchronizer(TClockEventListener* clock_event_listener)
      : trace_time_clock_id_(protos::pbzero::BUILTIN_CLOCK_BOOTTIME),
        clock_companion_(clock_event_listener) {}

  ~ClockSynchronizer() = default;
  ...
  PERFETTO_ALWAYS_INLINE base::StatusOr<int64_t> ToTraceTime(
    ...
    if (PERFETTO_UNLIKELY(!trace_time_clock_id_used_for_conversion_)) {
      clock_companion_->OnTraceTimeClockIdChanged(trace_time_clock_id_);
    }
    ...
    for (const auto& clock_ts : clock_timestamps) {
      ...
      if (domain.snapshots.empty()) {
        if (clock_ts.clock.is_incremental &&
          ...
          clock_companion_->OnInvalidClockSnapshot();
  ...
  base::Status SetTraceTimeClock(ClockId clock_id) {
    ...
    trace_time_clock_id_ = clock_id;
    clock_companion_->OnSetTraceTimeClock(clock_id);
 ...
 private:
  ...
  base::StatusOr<int64_t> ConvertSlowpath(
    ...
    clock_companion_->OnClockSyncCacheMiss();
  ...
  ClockSynchronizer::ClockPath FindPath(ClockId src, ClockId target) {
  ...
  PERFETTO_ALWAYS_INLINE int64_t ToHostTraceTime(int64_t timestamp) {
    ...
    //   return timestamp;
    // }
    if (PERFETTO_LIKELY(clock_companion_->IsLocalHost())) {
      // No need to convert host timestamps.
      return timestamp;
    }
  ...
  TClockEventListener* clock_companion_;
...
#endif  // SRC_TRACE_PROCESSOR_UTIL_CLOCK_TRACKER_H_
